 ...
 */
final record BeanScannerConstructors(Constructor<?> constructor, OperationType operationType) {
    ...
    static final ClassValue<BeanScannerConstructors> CACHE = new ClassValue<>() {

        /** {@inheritDoc} */
        protected BeanScannerConstructors computeValue(Class<?> implementation) {
            Constructor<?> executable = BeanScannerConstructors.getConstructor(implementation, true, e -> new IllegalArgumentException(e));
            return new BeanScannerConstructors(executable, OperationType.fromExecutable(executable));
        }
    };
    ...
    static void findConstructor(BeanScanner scanner, Class<?> beanClass) {
        ...
        if (scanner.bean.beanSourceKind == BeanSourceKind.CLASS && scanner.bean.beanKind != BeanKind.STATIC) {
            ...
            BeanScannerConstructors constructor = BeanScannerConstructors.CACHE.get(beanClass);
