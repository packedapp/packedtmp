 ...
 *
 */
final class BeanScannerVariable {
    ...
    static void resolveVariable(BeanScanner scanner, OperationSetup operation, Variable v, int index) {
        ...
        for (Annotation a1 : annotations) {
            ...
            if (hook != null) {
                BeanIntrospectorSetup ei = scanner.computeIntrospector(hook.extensionType());
        ...
        if (hook != null) {
            BeanIntrospectorSetup contributor = scanner.computeIntrospector(hook.extensionType());
