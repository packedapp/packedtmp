...
import app.packed.bean.operation.usage.TestItPublic.HttpMethodOperationMirror;
import app.packed.bean.operation.usage.TestItPublic.HttpRequest;
import app.packed.bean.operation.usage.TestItPublic.HttpResponse;
import app.packed.bean.operation.usage.TestItPublic.Router;
...
public class TestIt extends Extension<TestIt> {
    ...
    private BeanHandle<?> fb() {
        ...
        if (fb == null) {
            functionalBean = fb = bean().builder(BeanKind.FUNCTIONAL).build();
            // fb.setPrefix("fWeb")
        }
        return fb;
    }
