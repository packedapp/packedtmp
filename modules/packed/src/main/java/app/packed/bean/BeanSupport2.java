...
public class BeanSupport2 extends ExtensionSupport {
    ...
    public final <T, B extends BeanConfiguration<T>> B register(Realm agent, BeanDriver driver, B configuration, Class<T> implementation) {
        requireNonNull(implementation, "implementation is null");
        extension.wire(configuration, agent, implementation);
        throw new UnsupportedOperationException();
    }

    public final <T, B extends BeanConfiguration<T>> B registerChild(Realm agent, ComponentConfiguration parent, BeanDriver driver, B configuration,
