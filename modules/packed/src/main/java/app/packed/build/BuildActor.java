...
public /* value */ final class BuildActor {

    /** An application author. */
    // Was Application, Developer
    // Actor -> [Application, Extension]
    private static final BuildActor APPLICATION = new BuildActor(Extension.class);

    /** Interned realm. */
    // Until we get values in which case we is always interned
    static final ClassValue<BuildActor> INTERNED = new ClassValue<BuildActor>() {

        @Override
        protected BuildActor computeValue(Class<?> extensionClass) {
            ClassUtil.checkProperSubclass(Extension.class, extensionClass, "extensionClass");
            return new BuildActor(extensionClass);
        }
    };
    ...
    private BuildActor(@SuppressWarnings("rawtypes") Class extensionClass) {
     ...
     */
    public static BuildActor extension(Class<? extends Extension<?>> extensionClass) {
        // ExtensionModel.get().operative???
        return INTERNED.get(extensionClass);
    }

    /** {@return the application author.} */
    public static BuildActor application() {
        return APPLICATION;
    }
}
