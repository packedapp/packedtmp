...
public final class WireletContext {
    ...
    final IdentityHashMap<Class<? extends WireletPipeline<?, ?, ?>>, List<PipelinedWirelet<?>>> pipelines = new IdentityHashMap<>();
    ...
    private void apply(FixedWireletList wirelets) {
        for (Wirelet w : wirelets.toArray()) {
            if (w instanceof PipelinedWirelet) {
                ExtensionWireletPipelineModel pm = ExtensionWireletModel.of((Class<? extends PipelinedWirelet<?>>) w.getClass());
                pipelines.computeIfAbsent(pm.type, k -> new ArrayList<>()).add((PipelinedWirelet<?>) w);
...
//                throw new IllegalArgumentException("Wirelets of type " + StringFormatter.format(w.getClass()) + " are not supported");
    ...
    public void initializex(PackedExtensionContext pec, Class<? extends WireletPipeline<?, ?, ?>> etype) {
        List<PipelinedWirelet<?>> ewp = pipelines.get(etype);
        if (ewp != null) {
            ExtensionWireletPipelineModel m = ExtensionWireletModel.of((Class<? extends PipelinedWirelet<?>>) ewp.iterator().next().getClass());
    ...
    public void initialize(PackedExtensionContext pec) {
        for (var p : pec.model().pipelines.keySet()) {
            List<PipelinedWirelet<?>> ewp = pipelines.get(p);
            if (ewp != null) {
                ExtensionWireletPipelineModel m = ExtensionWireletModel.of((Class<? extends PipelinedWirelet<?>>) ewp.iterator().next().getClass());
