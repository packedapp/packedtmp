...
public final class ComponentConfigurationToComponentAdaptor implements Component {
    ...
    private final AbstractOldComponentConfiguration componentConfiguration;

    // Need to main any guest ancestor. As images must resolve in relation to it.
    private final List<PackedGuestConfiguration> pgc;

    private ComponentConfigurationToComponentAdaptor(AbstractOldComponentConfiguration componentConfiguration, List<PackedGuestConfiguration> pgc) {
    ...
    public final Collection<Component> children() {
        ...
        if (c == null) {
            if (componentConfiguration.children == null) {
                ...
                for (AbstractOldComponentConfiguration acc : componentConfiguration.children.values()) {
    ...
    private final Stream<Component> stream0(AbstractOldComponentConfiguration origin, boolean isRoot, PackedComponentStreamOption option) {
    ...
    private static ComponentConfigurationToComponentAdaptor of0(AbstractOldComponentConfiguration bcc, List<PackedGuestConfiguration> pgc) {
