 ...
 * {@link SimpleInjectorConfigurator#wireInjector(Injector, OldWiringOperation...)} or
 * 
 * <p>
...
public abstract class OldWiringOperation {

    static {
        BundleSupport.Helper.init(new BundleSupport.Helper() {

            @Override
            public List<OldWiringOperation> extractWiringOperations(OldWiringOperation[] operations, Class<?> type) {
                return ComposedWiringOperation.operationsExtract(operations, type);
            }

            /** {@inheritDoc} */
            @Override
            public void finishWireOperation(OldWiringOperation operation) {
                operation.onFinish();
            }

            @Override
            public Lookup lookupFromWireOperation(OldWiringOperation operation) {
                return operation.lookup;
            }

            /** {@inheritDoc} */
            @Override
            public void startWireOperation(OldWiringOperation operation) {
                operation.onStart();
            }
        });
    }
     ...
     */
    protected OldWiringOperation(MethodHandles.Lookup lookup) {
     ...
     * @see #compose(OldWiringOperation...)
     */
    public OldWiringOperation andThen(OldWiringOperation next) {
    ...
    static List<OldWiringOperation> expandRecursively(OldWiringOperation... operations) {
     ...
     * @see #andThen(OldWiringOperation)
     */
    public static OldWiringOperation compose(OldWiringOperation... operations) {
        return new ComposedWiringOperation(operations);
    }

    /** An operation that combines multiple operations. */
    static final class ComposedWiringOperation extends OldWiringOperation {

        /** The stages that have been combined */
        private final List<OldWiringOperation> operations;

        ComposedWiringOperation(OldWiringOperation... operations) {
        ...
        static List<OldWiringOperation> operationsExtract(OldWiringOperation[] operations, Class<?> type) {
            requireNonNull(operations, "operations is null");
            if (operations.length == 0) {
                return List.of();
            }
            ArrayList<OldWiringOperation> result = new ArrayList<>(operations.length);
        ...
        private static void operationsExtract0(OldWiringOperation o, Class<?> type, ArrayList<OldWiringOperation> result) {
            if (o instanceof ComposedWiringOperation) {
                for (OldWiringOperation ies : ((ComposedWiringOperation) o).operations) {
