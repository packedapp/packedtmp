 ...
 * <p>
 * Note that we create a new class inheriting from Factory0, this is done in order to capture type information.
 *
 * As an alternative to using the constructor of this class, one of the static factory methods can be used:
 *
 * <pre> {@code
 * Factory<Long> f = Factory0.of(System::currentTimeMillis, Long.class)).setDescription("Startup Time")}</pre> </pre>
 *
 * The last factory is functionally equivalent to the factory created in the first example.
 *
 * @param <T>
...
public abstract class Factory0<T> extends Factory<T> {
     ...
     *             if the type variable T could not be determined
     */
