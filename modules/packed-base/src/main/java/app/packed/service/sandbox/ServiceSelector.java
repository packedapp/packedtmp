...
public interface ServiceSelector<T> extends Predicate<ServiceDescriptor> {
    ...
    // ServiceConfiguration<List<T>> asList();
...
class S2 extends Extension {
    ...
    // extension.captureAll().OfType(G).intoSet();
    // CaptureAll.ofType(F).intoSet(extension)
    // CaptureAll.ofType(F).fromInjector(injector)

    // ServiceConfiguration<Set<T>> ServiceSelection.of(Foo.class).captureSet(extension);

    // injector.getAll(ServiceSelector)

    // A dynamic selection of services
    // ServiceSet ServiceSet.of(Doo);

    // ServiceSet / ServiceSelector / ServiceSelection
    // SS extends Predicate<ServiceDef>
    // SS.isSelected(ServiceDef)
    // SS.isSelected(key)

    // ss.checkNonEmpty();

    // ss.injectInto(componentConfiguration); // local Injection?
