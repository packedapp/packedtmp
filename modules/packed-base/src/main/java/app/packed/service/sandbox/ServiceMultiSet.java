...
public interface ServiceMultiSet<S> extends Iterable<S> {

    Set<Service> descriptors();

    // ServiceProvider????
    void forEach(BiConsumer<Service, ServiceProvider<S>> action);

    /**
     * Returns whether or not all services have a unique key. If these method returns false. The value returned by
     * {@link #size()} will be greater than the number of keys returned by {@link #keys()}.
     * 
    ...
    Set<ServiceProvider<S>> providers();
